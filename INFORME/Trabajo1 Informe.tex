...
\begin{figure}[!htb]
  \centering
  \includegraphics[scale=0.3]{FigurasUV/Rplot1.png}
...
\begin{figure}[!htb]
  \centering
  \includegraphics[scale=0.3]{FigurasUV/poisson.png}
  \caption{Frecuencia esperada vs observada de la distribución Poisson$(\lambda=1.99)$}\label{figura1}
\end{figure}
~\\ El estadistico de prueba de la prueba $\chi^2$ de Pearson nos dio $\chi_{obs}^{2}=\sum\limits_{i=1}^{k}\frac{(O_{i}-E_{i})^2}{E_{i}}=0.001783335$ y el $p-valor=P(\chi^{2}_{k-1}>\chi^{2}_{obs})=P(\chi^{2}_{9}>0.00178335)=1$. Por lo tanto, no rechazamos $H_{0}$ y concluimos que los datos generados por el método de la transformada inversa se distribuyen Poisson$(\lambda=1.99)$, esto quiere decir que el método de generación de datos utilizado realmente funciona.

\newpage\subsubsection{Distribucion Logistica:}
\subsubsection*{Prueba de bondad de ajuste Kolmogorov-Smirnov:}
